    ...
    public boolean checkInclusion(String s1, String s2) {
        ...
        while(l < s2.length() && r < s2.length()){
            ...
            if (!freqs.containsKey(s2.charAt(r))){
                if (freqsWindow.containsKey(s2.charAt(l)))
                    if (freqsWindow.get(s2.charAt(l)) == 0)
                        freqsWindow.remove(s2.charAt(l));
                    else
                        freqsWindow.put(s2.charAt(l), freqsWindow.get(s2.charAt(l))-1);

                if (freqsWindow.containsKey(s2.charAt(r)))
                    if (freqsWindow.get(s2.charAt(r)) == 0)
                        freqsWindow.remove(s2.charAt(r));
                    else
